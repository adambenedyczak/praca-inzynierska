<div>
<div class="card">
            <div class="card-header font-weight-bold">{{ __('object.vehicle.create.new') }}</div>
    ...
    <form action="{{ route('objects.store') }}" method="POST" class="needs-validation" novalidate>
        ...
        <div class="form-group form-floating {{ $errors->has('object_name') ? 'has-error' : '' }}">
            ...
            <div class="form-floating mt-3">
                <label for="unit">{{ __('object.vehicle.create.unit_name') }}</label>
                <select name="object_unit"
                        wire:model="object_unit"
                        class="form-control" required >
                    <option value="">{{ __('object.vehicle.create.choose_unit') }}</option>
                    @foreach ($workTimeUnits as $unit)
                        <option value="{{ $unit->id }}" @if($unit->short == "km") selected @endif>
                            {{ $unit->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="card">
            ...
            {{ __('object.vehicle.create.details') }}
            </div>
