<div>
<div class="card">
            <div class="card-header font-weight-bold">{{ __('object.trailer.create.new') }}</div>
    ...
    <form action="{{ route('objects.store') }}" method="POST" class="needs-validation" novalidate>
        ...
        <div class="form-group form-floating {{ $errors->has('object_name') ? 'has-error' : '' }}">
            <div class="form-floating">
                <label for="floatName">{{ __('object.trailer.create.name') }}</label> 
            ...
            <div class="form-floating mt-3">
                <label for="unit">{{ __('object.trailer.create.unit_name') }}</label>
                <select id="unit" name="object_unit"
                        wire:model="object_unit"
                        class="form-control" required >
                    <option value="">{{ __('object.trailer.create.choose_unit') }}</option>
                    @foreach ($workTimeUnits as $unit)
                        <option value="{{ $unit->id }}" @if($unit->short == "") selected @endif>
                            {{ $unit->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="card">
            ...
            {{ __('object.trailer.create.details') }}
            </div>
                ...
                <div class="form-row align-items-center my-2">
                    <div class="col-sm-4 mb-1">
                        <select name="addDetails[{{$index}}][detail_type_id]"
                                ...
                                class="form-control" >
                            <option value="">{{ __('object.trailer.create.choose_type') }}</option>
                    ...
                    <input type="text"
                        ...
                        class="form-control"
                        placeholder="{{ __('object.trailer.create.put_value') }}"
                ...
                <div class="form-row align-items-center my-2">
                    <div class="col-sm-4 mb-1">
                        <input type="text"
                            ...
                            class="form-control"
                            placeholder="{{ __('object.trailer.create.put_name') }}"
